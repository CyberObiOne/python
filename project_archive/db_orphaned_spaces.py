...
def confluence_spaces():
        try:
                connection = psycopg2.connect(user = os.environ.get("CONFLUENCE_LOGIN"),
                                  password = os.environ.get("CONFLUENCE_PASSWORD"),
                                  host = os.environ.get("HOST"),
                                  port = "5432",
                                  database = os.environ.get("CONFLUENCE_DATABASE"))
                ...
                cursor.execute("""select json_agg(hash::jsonb || projects::jsonb) from
                                 (SELECT s.SPACEKEY, s.SPACENAME,
                                  ...
                                  WHERE p.PERMTYPE = 'SETSPACEPERMISSIONS' and s.spacestatus != 'ARCHIVED' and c.active ='F'
